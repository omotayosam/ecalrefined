...
class Products
{
	...
	public function getProducts(){
		$q = $this->con->query("SELECT i.id, i.item_name, i.price, i.no_item_left, i.item_details,i.currency, i.item_pic, i.item_keywords, c.cat_title, c.cat_id, b.brand_id, b.brand_title FROM items i JOIN categories c ON c.cat_id = i.item_category JOIN brands b ON b.brand_id = i.item_class");
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				// $img_path = $_SERVER['DOCUMENT_ROOT']."/ecalrefined/wwwroot/img/".$uniqueImageName;
				$img_path = "../../wwwroot/img/".$uniqueImageName;
				$img_path_db = "wwwroot/img/".$uniqueImageName;
				if (move_uploaded_file($file['tmp_name'], $img_path)) {
					
					$q = $this->con->query("INSERT INTO `items`(`item_category`, `item_class`, `item_name`, `price`,  `no_item_left`, `item_details`, `item_pic`, `item_keywords`) VALUES ('$category_id', '$brand_id', '$product_name', '$product_price',  '$product_qty', '$product_desc', '$img_path_db', '$product_keywords')");
	...
	public function editProductWithImage($pid,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				$uniqueImageName = time()."_".$file['name'];
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecalrefined/wwwroot/img/".$uniqueImageName)) {
					
					$q = $this->con->query("UPDATE `items` SET 
										`item_category` = '$category_id', 
										`item_class` = '$brand_id', 
										`item_name` = '$product_name',
										`price` = '$product_price', 
										`no_item_left` = '$product_qty',  
										`item_details` = '$product_desc', 
										`item_pic` = '$uniqueImageName', 
										`item_keywords` = '$product_keywords'
										WHERE id = '$pid'");
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			$q = $this->con->query("DELETE FROM items WHERE id = '$pid'");
			if ($q) {
...
if (isset($_POST['DELETE_PRODUCT'])) {
	...
	if (isset($_SESSION['admin_id'])) {
		if(!empty($_POST['pid'])){
			$pid = $_POST['pid'];
			echo json_encode($p->deleteProduct($_POST['pid']));
			exit();
		}else{
